    ...
    def process_prompt(self, user_prompt):
        ...
        if self.rag_retriever is not None and self.rag_enabled:
            try:
                ...
                self._info(f">> RAG retrieval failed, continuing without context: {e}")
                rag_context = None
            if rag_context:
                augmented = f"{rag_context}\n\nUser question: {user_prompt}"
                messages_to_send = self.messages[:-1] + [{'role': 'user', 'content': augmented}]
                if self.verbose:
                    self._info(f">> RAG context injected:\n{rag_context}")
